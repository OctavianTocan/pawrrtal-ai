...
- Effect TypeScript strangler workspace is `backend-ts/`: **`@pawrrtal/api-core`** (`packages/api-core` — HttpApi groups, `Domain.ts`, `Errors.ts`, root `Api.ts`) and **`@pawrrtal/api`** (`apps/api` — `Http*Live`, `Service`, `Repo`, `Policy`, `App.ts`, `Main.ts`, `Modules/Layers.ts`, `Infrastructure/`). Pin npm `effect@4.0.0-beta.74` (and matching platform packages); cite APIs from git submodule `backend/vendor/effect-smol` (`ai-docs/`, especially `51_http-server/10_basics.ts`). Do not `file:`-link effect-smol (internal `workspace:^`). `backend/vendor/effect` is Effect v3 only — not a v4 branch. Vendor roles: `backend/vendor/README.md`. Python on `:8000` stays canonical until route parity. Install/typecheck: `just install-backend-ts`, `just typecheck-backend-ts`.
