...
- For Effect TypeScript work, treat `backend/vendor/effect-smol` as the v4 API source of truth (examples and `ai-docs`); do not assume npm Effect v3 import paths without checking the vendor tree.
...
- Effect TypeScript strangler workspace is `backend-ts/`: `packages/api-core` (shared HttpApi/schemas/errors), `apps/api` (Node server). Runtime deps use npm `effect@4.0.0-beta.74` (Effect v4, not v3); read implementations in git submodule `backend/vendor/effect-smol` (avoid `file:` links — submodule uses internal workspaces). Structural reference: `backend/vendor/comcom`. Python on `:8000` stays canonical until route parity. Install/typecheck: `just install-backend-ts`, `just typecheck-backend-ts`.
