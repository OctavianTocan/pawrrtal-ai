...
- **Rule (bean writing style)**: Write bean bodies so a junior engineer (or a future agent with zero context) can act on them without re-reading the codebase. Plain words over jargon. Use:
    ...
    - `## File` / `## Files` section naming the exact path(s) to touch.
    - **Numbered steps** for actions; **tables** for settings / config / test cases; **fenced code blocks** for every snippet, signature, and shell command.
    - **Folder-tree diagrams** when creating multiple files in a new package.
    - Short bolded labels (`**Safety rule:**`, `**Off by default.**`) for scannability.
    - For epics: a `## How it works` numbered walkthrough + a `## Rules` block (off-by-default flags, must-not-break invariants, where it lives).
    - Avoid run-on paragraphs of comma-separated requirements — break them into a list or table. If a bean reads like a wall of text, it's wrong.
    - When updating bean bodies in bulk, write each new body to a temp `.md` file and apply via `beans update <id> --body-file /tmp/...md`.
- **AI Rules (`.claude/rules/`)**: Most rules are vendored from [github.com/OctavianTocan/claude-rules](https://github.com/OctavianTocan/claude-rules) into `.claude/rules/`; each file uses YAML frontmatter with `paths` globs so rules apply only for matching files. The project-specific rule sets (clean-code, github-actions) are bundled in the Pawrrtal plugin under `.cursor/plugins/pawrrtal/rules/clean-code/` and `.cursor/plugins/pawrrtal/rules/github-actions/`.
...
This is a public repo wired to a self-hosted runner pool on Octavian's VPS. CI is **scoped to OctavianTocan** for safety, not by accident — every workflow you create or modify must obey the rules in `.cursor/plugins/pawrrtal/rules/github-actions/octaviantocan-only-and-self-hosted-runner.mdc`. The short version:
...
- **Documented exception:** `rebase.yml` uses `pull_request_target` and never runs PR code; it relies on `author_association` instead of the actor gate. See `.cursor/plugins/pawrrtal/rules/github-actions/safe-pull-request-target.mdc`.
...
- **Preserve Documentation**: NEVER remove existing docstrings, JSDoc comments, or explanatory comments when modifying code. Only remove documentation if the code it documents is being deleted, or update it if your changes make it inaccurate. See `.cursor/plugins/pawrrtal/rules/clean-code/preserve-documentation.mdc` for detailed rules.
...
- **Provider-agnostic tools**: model-visible tool implementation helpers live in `backend/app/tools/`, provider adapters live in `backend/app/providers/`, and the agent-owned capability facade lives in `backend/app/agents/tool_capabilities/`. Providers translate `AgentTool[]` → SDK format and never import tool modules themselves. Tool composition for a turn lives in `backend/app/agents/tool_surface.py`, called by the Turn Pipeline, not in `backend/app/chat/router.py`. Enforced by `scripts/check-no-tools-in-providers.py`. See `.claude/rules/architecture/no-tools-in-providers.md`.
- **Multi-file features live in packages, not flat siblings with redundant prefixes**: when a provider (or any feature / subsystem) grows past one module, collect its files into a `<feature>/` directory with role-named modules (`provider.py`, `client.py`, `events.py`, `fs.py`) and a `__init__.py` that re-exports the public surface. Drop the feature-name prefix from the inner files — the directory already carries the namespace. Single-file features stay flat. `backend/app/providers/gemini_cli/` is the template; `frontend/features/<feature>/` already follows the convention. Provider packages such as `backend/app/providers/claude/`, `backend/app/providers/gemini/`, `backend/app/providers/xai/`, and `backend/app/providers/opencode_go/` are the current shape to preserve. See `.cursor/plugins/pawrrtal/rules/clean-code/package-multi-file-features.mdc`.
...
- See `.cursor/plugins/pawrrtal/rules/clean-code/limit-nesting-depth.mdc` for guidance on flattening with guard clauses and helper functions.
...
- **GitHub Actions Rules (`.cursor/plugins/pawrrtal/rules/github-actions/`)**: Strict context and design patterns to follow when creating or modifying CI/CD workflows and actions.
- **Clean Code Rules (`.cursor/plugins/pawrrtal/rules/clean-code/`)**: Universal rules for function design, naming conventions, named constants, Python logging/exception narrowing, and code structure. Your generated code must adhere to these principles (KISS, DRY, single-responsibility, meaningful naming).
...
- Local dev runs on plain localhost: Next.js on `http://localhost:53001`, Python FastAPI on `http://localhost:8000` (`DEV_BACKEND_PORT` in `scripts/dev-ports.ts`), Effect TS strangler on `http://localhost:8001` (`DEV_BACKEND_TS_PORT`). `just dev` / `dev.ts` starts all three together. No HTTPS, no proxy, no special hostnames. For raw `uvicorn`, unset shell `DATABASE_URL` or use `just dev` (branch-scoped SQLite) — a stray Postgres `DATABASE_URL` breaks local Python startup.
- Effect TypeScript strangler workspace is `backend-ts/`: **`@pawrrtal/api-core`** (`packages/api-core` — HttpApi groups, `Domain.ts`, `Errors.ts`, root `Api.ts`) and **`@pawrrtal/api`** (`apps/api` — `Http*Live`, `Service`, `Repo`, `Policy`, `App.ts`, `Main.ts`, `Modules/Layers.ts`, `Infrastructure/`). Auth runtime lives in `apps/api/src/Modules/Authentication/` (SessionStore, HttpAuthLive). `SessionStore` still validates sessions via HTTP to Python `GET :8000/users/me` until Session-in-Effect lands. Provide `HttpAuthLive` on protected group layers (e.g. `HttpProjectsLive.pipe(Layer.provide(...))`), not only `CoreModulesLive.mergeAll`. In cookie middleware, map `SessionStoreError` → `AuthenticationError` at `sessionStore.lookup()`, not on `provideService`. Private-deployment authz parity uses `ALLOWED_EMAILS` (comma-separated, case-insensitive); empty/unset admits any authed user; non-empty returns 403 with message `This Pawrrtal deployment is private.` for unlisted emails (`get_allowed_user`). HttpApi group middleware: last `.middleware()` is outer; allowlist middleware that `yield* CurrentUser` must be inner, authentication outer. System health is `GET /api/v1/health` (`SystemApi` topLevel), not `/api/v1/system/health`. Pin npm `effect@4.0.0-beta.74` (and matching platform packages); cite APIs from git submodule `backend/vendor/effect-smol` (`ai-docs/`, especially `51_http-server/10_basics.ts`). Do not `file:`-link effect-smol (internal `workspace:^`). `backend/vendor/effect` is Effect v3 only — not a v4 branch. Vendor roles: `backend/vendor/README.md`. Python on `:8000` stays canonical until route parity. Install/typecheck: `just install`, `just typecheck`; repo-wide `just check` is VCS-scoped (changed files only). Full `backend-ts` lint: `cd backend-ts && bun run check` (`backend-ts/biome.json` sets `vcs.enabled: false`). **Migration direction (003 overhaul):** `SessionStore` (cookie→Python `/users/me`) becomes a native `ProfileStore` (`X-Pawrrtal-Profile` + `Tailscale-User-Login` trust), and `ALLOWED_EMAILS` becomes `allowed_tailscale_logins`; the in-flight cookie-session slice is a learning exercise superseded by a follow-up profiles slice that reuses the same `HttpApiMiddleware` scaffolding (`specs/003-pawrrtal-overhaul/research.md` §11). Persistence also pivots to files-first (`FileStore`, no `@effect/sql`/SQLite for app data) per §12.
- In Pawrrtal domain language, **Projects** means authenticated conversation folders at `/api/v1/projects` (`backend/app/projects/` in Python), not Tasks UI mocks or chat/agent endpoints; the Effect migration pilot is that CRUD slice on `:8001`.
- Frontend → backend cookie auth works because both run on the same host (`localhost`); cookies ignore ports, so `Set-Cookie` from `:8000` is visible to fetches from `:53001` with `credentials: 'include'`. Use `COOKIE_SAMESITE=lax` and `COOKIE_SECURE=false` in dev. **Migration direction (003 overhaul):** cookie auth is retired in favor of **profiles** — every request carries `X-Pawrrtal-Profile: <id>` (+ `Authorization: Bearer <token>` only for password-locked profiles); see `specs/003-pawrrtal-overhaul/research.md` §11. True for the current Python world; do not extend cookie auth in new `003` slices.
...
- `pr-scope-audit` — use before answering whether work is really in a PR, before claiming a long goal is complete, or when repo work and VPS operations were mixed together.
- `live-ops` — use for live deployment, Cloudflared, Telegram, database, browser, shutdown, and real integration verification.
- `runner-ops` — use before touching self-hosted GitHub Actions runners; runner state must stay on `/mnt/HC_Volume_105512717/github-runners/` and self-hosted jobs must be locked to Octavian-only workflow gates.
- `extension-boundaries` — use before changing providers, channels, tools, plugins, subagents, context providers, or turn orchestration.
