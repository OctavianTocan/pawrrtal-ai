...
Three concerns are exercised:
  ...
  * Manifest registration — the bundled plugin manifest declares a
    single ``notion_cli`` CLI capability and workspace-scoped env metadata.
...
class TestRegistration:
    def test_bundled_manifest_declares_notion_cli_capability(self) -> None:
        manifest_path = Path(__file__).resolve().parents[1] / "plugins" / "notion" / "plugin.json"
        manifest = validate_plugin_manifest(
            manifest_path.read_text(encoding="utf-8"),
            source_type="bundled",
        )

        assert manifest.id == "notion"
        assert [capability.id for capability in manifest.capabilities] == [
            "notion_cli",
            "notion_settings",
        ]
        assert [spec.name for spec in manifest.all_env_specs()] == ["NOTION_API_KEY"]
        assert fingerprint_plugin(manifest_path.parent, manifest)

    def test_legacy_tool_factory_still_builds_notion_cli_shape(self) -> None:
        tool = make_ntn_tool(
            ToolContext(
