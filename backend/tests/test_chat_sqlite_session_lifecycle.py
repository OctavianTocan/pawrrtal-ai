...
async def test_sse_done_frame_waits_for_turn_finalization() -> None:
    ...
    """
    from app.turns.pipeline import _finalizing_stream
...
async def test_finalize_turn_leaves_message_complete_on_cost_write_failure(
    ...
    # Patch where the symbol is looked up — ``finalize`` imported it
    # at module load, so patching ``app.turns.pipeline.cost`` would miss
