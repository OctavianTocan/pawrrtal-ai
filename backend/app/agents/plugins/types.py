...
class EnvKeySpec:
    ...
    allowlist in :mod:`app.infrastructure.keys` is still the source of truth);
    ...
    Attributes:
        name: The env-var name as stored in the workspace ``.env``
            ...
            entry in :data:`app.infrastructure.keys.OVERRIDABLE_KEYS`.
...
class ToolContext:
    ...
    Attributes:
        workspace_id: The active workspace UUID. Plugins resolve env
            keys via :func:`app.infrastructure.keys.resolve_api_key` against this
            workspace.
...
class PreTurnHookContext:
    """Context passed to pre-turn plugins before the main agent runs."""
