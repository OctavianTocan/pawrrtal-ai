...
# them stay in sync.
ACCESS_TOKEN_KEY = "XAI_OAUTH_ACCESS_TOKEN"
REFRESH_TOKEN_KEY = "XAI_OAUTH_REFRESH_TOKEN"
EXPIRES_AT_KEY = "XAI_OAUTH_EXPIRES_AT"
...
async def _resolve_oauth_token(workspace_root: Path) -> str | None:
    ...
    access_token = env.get(ACCESS_TOKEN_KEY, "").strip()
    refresh_value = env.get(REFRESH_TOKEN_KEY, "").strip()
    expires_at_raw = env.get(EXPIRES_AT_KEY, "").strip()
