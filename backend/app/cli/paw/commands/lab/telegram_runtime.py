"""Runtime helpers for ``paw lab telegram`` commands."""
...
from .provider_selection import extract_models, select_fast_provider_models
from .storage import new_run_id


async def telegram_chat_payload(
    state: PersonaState,
    *,
    model_id: str,
    turns_path: Path,
...
async def telegram_media_payload(
    state: PersonaState,
    *,
    model_id: str,
    text: str,
...
async def telegram_providers_payload(
    state: PersonaState,
    *,
    text: str,
    image_path: Path | None,
    ...
    """Run the same media test against one selected model per provider host."""
    media_payload = _build_media_payload(
        text=text,
        image_path=image_path,
        voice_note_path=voice_note_path,
    ...
    async with PawClient(state, timeout=request_timeout) as client:
        models = extract_models((await client.request("GET", "/api/v1/models")).json())
    selected = select_fast_provider_models(models, include_hosts=include_hosts)
    if not selected:
        raise LocalError("No authenticated provider models matched the requested host filter.")

    provider_runs = [
        await _telegram_provider_payload(
            state,
            row=row,
    ...
    return {
        "run_id": new_run_id("telegram-media-providers"),
        "kind": "telegram-media-providers",
        "media": _media_summary(media_payload),
        "selected_models": selected,
        "provider_runs": provider_runs,
        "summary": {
            "providers": len(provider_runs),
            "failed_providers": sum(1 for run in provider_runs if not run.get("ok")),
            "max_client_duration_ms": _max_provider_duration(provider_runs),
        },
    }


async def _telegram_provider_payload(
    state: PersonaState,
    *,
    row: dict[str, Any],
    ...
    """Run one provider row and return either a child payload or a failure row."""
    model_id = str(row.get("model_id") or row.get("id"))
    host = str(row.get("host") or "unknown")
    try:
        child = await _send_model_media_payload(
            state,
            model_id=model_id,
            media_payload=media_payload,
            verbose_level=verbose_level,
    ...
    return {"ok": True, "host": host, "model_id": model_id, "run": child}


async def _send_model_media_payload(
    state: PersonaState,
    *,
    model_id: str,
    media_payload: dict[str, Any],
    ...
    """Send one media payload through a fresh Telegram conversation."""
    control_messages = _script_messages(
        model_id=model_id,
        turns=[],
        new=True,
...
def _max_provider_duration(provider_runs: list[dict[str, Any]]) -> int | None:
    """Return the slowest child media turn duration across provider runs."""
    values: list[int] = []
    for provider_run in provider_runs:
        child = provider_run.get("run")
        if not isinstance(child, dict):
            continue
        summary = child.get("summary")
        if not isinstance(summary, dict):
            continue
        duration = summary.get("max_client_duration_ms")
        if isinstance(duration, int):
            values.append(duration)
    return max(values) if values else None


def render_telegram_chat(payload: dict[str, Any]) -> str:
...
def render_telegram_providers(payload: dict[str, Any]) -> str:
    """Render a compact provider-matrix report."""
    summary = payload.get("summary") or {}
    return (
        f"run_id: {payload.get('run_id')}\n"
        f"providers: {summary.get('providers')}\n"
        f"failed_providers: {summary.get('failed_providers')}\n"
        f"max_client_duration_ms: {summary.get('max_client_duration_ms')}\n"
        f"run_path: {payload.get('run_path')}"
    )


def raise_on_provider_failures(payload: dict[str, Any]) -> None:
    """Exit non-zero after persisting provider matrix results if any failed."""
    if payload.get("kind") != "telegram-media-providers":
        return
    summary = payload.get("summary")
    failed = summary.get("failed_providers") if isinstance(summary, dict) else 0
    if isinstance(failed, int) and failed > 0:
        raise LocalError(
            f"Telegram media provider matrix failed for {failed} provider(s).",
            hint=f"Inspect run log: {payload.get('run_path')}",
        )
