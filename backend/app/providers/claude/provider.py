...
# the *shared* ``app.agents.system_prompt.DEFAULT_AGENT_SYSTEM_PROMPT``
        ...
        *,
        ...
        Args:
            ...
            workspace_root: Absolute path from the ``workspaces.path`` DB
                ...
                :func:`app.infrastructure.keys.resolve_api_key`.
        """
        ...
        *,
        ...
        Args:
            ...
            agent_tools: Cross-provider :class:`AgentTool` list assembled
                ...
                MCP server via
                :mod:`app.providers._claude_tool_bridge` and mounted
        ...
        # ``turns.pipeline.workspace_context.workspace_system_prompt`` already
        ...
        if mcp_servers:
            ...
            # We auto-approve every call inside our ``mcp__pawrrtal__*``
            # namespace via :func:`auto_approve_bridge_tools`.  The
            # whitelist on ``tools=`` is necessary but not sufficient —
            # without a ``can_use_tool`` callback the SDK blocks every
