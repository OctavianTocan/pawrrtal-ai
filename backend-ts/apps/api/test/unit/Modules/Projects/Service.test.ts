...
class ProjectsRepoTestRef extends Context.Service<ProjectsRepoTestRef, Ref.Ref<ReadonlyArray<Project>>>()(
...
const ProjectsRepoTest: Layer.Layer<ProjectsRepo, never, ProjectsRepoTestRef> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const insert = Effect.fn("ProjectsRepoTest.insert")(function* (input: { name: string; user_id: UserId }) {
    ...
    const update = Effect.fn("ProjectsRepoTest.update")(function* (id: ProjectId, userId: UserId, name: string) {
      const now = yield* DateTime.now
      const ts = DateTime.formatIso(now)
      const all = yield* Ref.get(store)
    ...
    const remove = Effect.fn("ProjectsRepoTest.delete")(function* (id: ProjectId, userId: UserId) {
      const all = yield* Ref.get(store)
