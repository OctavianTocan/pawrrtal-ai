...
const sqliteDbFilename = Config.string("SQLITE_DB_FILENAME").pipe(Config.withDefault(DEFAULT_SQLITE_DB_FILENAME))
...
export const DatabaseConfig: Config.Wrap<SqliteFilenameConfig> = {
  filename: Config.all({ databaseUrl, sqliteDbFilename }).pipe(
    Config.map(({ databaseUrl: url, sqliteDbFilename: filename }) => resolveFilename(url, filename))
  )
}
