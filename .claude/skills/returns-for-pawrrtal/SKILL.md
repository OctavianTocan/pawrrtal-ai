...
paths:
  ...
  - "backend/app/crud/**/*.py"
  - "backend/app/core/lcm/**/*.py"
  - "backend/app/channels/_telegram_finalize.py"
---
...
**Don't blanket-adopt.** Pilot it on **one surgical surface** where many callees funnel into a single caller, then re-evaluate after 4 weeks. That surface is the **provider seam** (`backend/app/core/providers/*`). A second candidate is **`backend/app/crud/*`** (replace `Optional[Row]` with `Maybe[Row]`).

Everything else — FastAPI routes, pydantic-adjacent code, schedulers, settings, voice, transcribers — keeps the existing exception idiom. The mypy plugin should be **scoped via `mypy.ini` overrides**, not enabled project-wide.
...
# backend/app/api/chat.py (caller orchestrates)
...
`returns` ships a mypy plugin (`returns.contrib.mypy.returns_plugin`) that enforces container-aware type narrowing. **Do NOT enable globally.** Add scoped overrides in `backend/pyproject.toml`:

```toml
[tool.mypy]
plugins = []  # plugin disabled globally

[[tool.mypy.overrides]]
module = ["app.core.providers.*"]
plugins = ["returns.contrib.mypy.returns_plugin"]
```

This keeps mypy fast everywhere else and avoids spurious findings in code that doesn't use the containers.
...
3. **Add scoped mypy override** for that one provider file.
...
- Did the pilot reduce backend bugs in the provider path? (Run `git log --oneline backend/app/core/providers/litellm_provider.py` and count fixes since pilot started vs the prior quarter.)
...
- Cloned vendor source: `/tmp/claude/research/returns/` (read-only research checkout)
