...
function makeDomainCliSkill(root: CommandMetadata): SkillFragment {
  return {
    ...
    body: [
      ...
      '## Rules',
      '',
      '- Use `effect@4.0.0-beta.92` and `@effect/platform-bun@4.0.0-beta.92` unless the spec is updated.',
      '- Use Bun runtime services for CLI execution; do not add `@effect/platform-node` or `node:*` imports to `packages/paw-cli/src`.',
      '- Keep command modules under `src/Modules/<Name>/` with `Domain.ts`, `Command.ts`, and focused helpers as needed.',
      ...
      '- Use `@effect/vitest` for tests that directly exercise Effect values or services.',
