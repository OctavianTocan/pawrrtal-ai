...
> **⚠️ SUPERSEDED — persistence/runtime substrate (2026-06-27).** The **files-first / no-database** decision throughout this plan (the `Storage:` and `Constraints:` lines, Phase 0 item 6, and the Step 7 persistence cutover) is **superseded** by the ADR `frontend/content/docs/handbook/decisions/2026-06-27-rivet-postgres-electric-hatchet-substrate.mdx`. The new substrate: live session state lives in a **per-conversation Rivet actor** running **Pi unforked**; the **queryable record lives in Postgres** (the API is its sole writer); **Electric** syncs Postgres read-models to all devices; **Hatchet** runs system-level durable work. The substrate-agnostic `SessionStore`/`EventStreamStore` **ports survive** — only their realization changes (per-actor SQLite + a Postgres projection instead of `$PAWRRTAL_DATA` JSONL). These sections are rewritten only after the validating spike; until then treat the ADR as the source of truth for persistence + runtime.
...
**Storage**: *(⚠️ SUPERSEDED by the 2026-06-27 substrate ADR — see banner above; the text below is retained for history.)* **PURE FILES — no database at all.** Conversations, profiles, projects, memory, config, and the `.agent/` workspace tree are all plain files under a single **`$PAWRRTAL_DATA`** root (a **git repo** for backups/history). NO Postgres, NO SQLite, NO Alembic for app data. Conversations are a per-conversation `meta.json` + append-only `messages.jsonl` (the `chat_messages` rows map 1:1 by `ordinal`); ledgers/audit/cost become append-only JSONL logs. **Search = ripgrep/scan over the JSONL — there is no index** (slower-search-at-scale tradeoff explicitly accepted). A **core-light `FileStore` service over effect-smol `FileSystem.ts` (+ `KeyValueStore.layerFileSystem(dir)`)** owns persistence. Alembic stops owning app data; a one-shot SQLAlchemy→file-tree exporter writes the file tree once.
...
**Constraints**: **Default fully self-hosted** (Clarification Q1) — self-hosted Infisical, own-infra sandboxes, **no external AI gateway**. ~~**Files-first persistence (no DB)** — all app data lives in the `$PAWRRTAL_DATA` file tree via `FileStore`; no Postgres/SQLite/Alembic.~~ *(⚠️ SUPERSEDED by the 2026-06-27 substrate ADR: Rivet per-conversation actors + Postgres + Electric + Hatchet; still self-hosted, but Postgres-backed.)* **Trusted users only** (Q2) — agent/model-generated *code* still sandbox-isolated (**`local-confined` default; Docker+gVisor/Kata/E2B opt-in tiers**). **Total migration** (Q3) — no Python in the end-state.
...
6. **Persistence + migration** — *(⚠️ SUPERSEDED by the 2026-06-27 substrate ADR — Rivet actors + Postgres + Electric + Hatchet replace the no-DB model; the original text is kept for history.)* ~~**files-first (`FileStore` over effect-smol `FileSystem.ts`)**; **NO database** (search = ripgrep/scan over the JSONL, no index); **no Postgres/SQLite/Alembic for app data**; the **`$PAWRRTAL_DATA` data root is a git repo**~~; extraction order Projects → read-only CRUD → paw CLI → message-write → streaming chat last.
...
- **Step 7 — Capability migration (strangler slices)**: read-only CRUD (workspaces, conversation metadata) → message-write slices → **streaming chat/turn last** (highest risk; flip per-route only after harness + parity gates). *(⚠️ The **persistence cutover** below is SUPERSEDED by the 2026-06-27 substrate ADR: instead of a file-tree export, the cutover moves conversations onto per-conversation Rivet actors with a Postgres projection synced by Electric; sequencing — read-only CRUD → message-write → streaming-last — still holds.)* ~~Includes the **persistence cutover**: introduce `FileStore`, run the **one-shot SQLAlchemy→file-tree exporter**, and **retire Alembic for app data** — **NO search-index rebuild** (there is no index; search is ripgrep/scan over the JSONL).~~
