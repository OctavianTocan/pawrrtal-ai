...
> **Persistence/runtime substrate (2026-06-27, spike-validated).** Persistence is the four-store split from the ADR `frontend/content/docs/handbook/decisions/2026-06-27-rivet-postgres-electric-hatchet-substrate.mdx`, **proven end-to-end by the `spikes/rivet-pi-electric` M1–M9 spike** (`rivetkit` 2.3.2 — past the RC the ADR flagged as its top risk; both halves of that risk, state durability and alarm durability, now proven): a **per-conversation Rivet actor** (running **Pi unforked**) for live session state · **Postgres** as the API-owned queryable record · **Electric** for read-path sync to all devices behind an identity-scoped gatekeeper · **Hatchet** for system-level durable work. The SDK-boundary, kernel-wraps-Pi, ports, profiles/Tailscale-identity, and sandbox decisions are **unchanged** — only the persistence realization is fixed. The standalone-SDK acceptance criteria (a `paw new` two-file project running over a **local store** with no Pawrrtal services) still hold; that local store is the SDK's reference impl, not the app's substrate.
...
- Q: How is application data persisted? → A: **A four-store substrate (spike-validated 2026-06-27).** Live session state (transcript + turn/queue/per-session-cron state) lives in a **per-conversation Rivet actor** running **Pi unforked**, persisted to the actor's own store and streamed over its WebSocket; the **cross-cutting queryable record** (conversation list/metadata, profiles, projects, automations, integrations, settings, search) lives in **Postgres** with the **API as sole writer**; **Electric** syncs those read-models to every device behind an identity-scoped gatekeeper; **Hatchet** runs system-level durable work. **Single-writer-per-store** is the invariant (actor owns its session, API owns Postgres, actor↔API over RPC). *(This replaced an earlier files-first/no-database model; see the substrate ADR for the rejected alternative and why audience growth + multi-device live sync + v1 search moved the decision.)*
...
- Q: What is the core agent loop built on? → A: **The Pi harness as the core loop** — the kernel **wraps `@earendil-works/pi-agent-core` (`agentLoop`, MIT, TypeScript) in Effect v4** rather than hand-rolling the loop, used **unforked** (forking would defeat the purpose). Pi is the **one sanctioned upstream dependency** inside the SDK boundary; **publishing the SDK later with Pi as a dependency is accepted.** Pi's own `pi-ai` provider layer is the core model abstraction (supersedes the earlier `@effect/ai`-as-core lean). The build-enforced app→SDK arrow and the no-app/runtime-imports rule are unchanged. (See Story 1, FR-002, Constraints.)

### Session 2026-06-29

- Q: Should Pawrrtal copy NanoClaw's agent-native setup/customization posture, where the repo teaches Claude Code how to install, recover, and modify the system? → A: **Yes, with Pawrrtal's config files kept deliberately.** Pawrrtal MUST have a deterministic setup path plus an agent-readable recovery/customization path: the README/AGENTS/skills/CLI teach a coding agent how to go from a fresh checkout to a working private instance, resume failed setup, add only the requested providers/channels/capabilities, and safely modify code/config. Unlike NanoClaw's "no configuration sprawl" stance, Pawrrtal keeps **typed, schema-backed config files** for durable operator state; customization is still agent-native, but agents edit validated config or small code modules instead of hiding decisions in chat.
...
The thin core is not just *Pawrrtal's* kernel — it is a **standalone, eventually-publishable agent-building SDK + CLI**, and **Pawrrtal is its first consumer**. The layering is named explicitly: a generic **framework/harness/runtime** (the agent turn loop — **the Pi harness `@earendil-works/pi-agent-core` (`agentLoop`, MIT) wrapped in Effect v4**, used **unforked**, plus compaction, the four shared contracts, and the ports as interfaces only) lives *below* the Pawrrtal **app/host**, and the dependency arrow is **build-enforced one-way: the app depends on the SDK; the SDK imports no app/runtime code** (Pi is the **one sanctioned upstream dependency** the kernel wraps; the eventual published SDK simply carries Pi as a dependency). Around the kernel sit a **thin foundation and a small set of packages in clean namespace layers** (the `@platform/*` foundation, the one-loop `kernel`, the `api-core` contract, the generated `api-client`, the uniform per-SDK `@clients/*` wrappers, and a few product packages), where **capabilities are added as data or small uniform packages — never by fattening the core**: external SDKs/providers/integrations are uniform wrapper packages; team-curated catalogs (providers, integrations, MCP) are declarative registry data; user-added skills/agents are stored rows injected at runtime; only core business is in-trunk code. The partition is **code vs. data** — generic loop machinery is SDK; anything *this*-agent / *this*-provider / *this*-channel specific is config. Adding a capability is "**drop a file**," never "edit the kernel." Pawrrtal-specific concerns (profiles, Tailscale trust, the persistence substrate — Rivet actors + Postgres + Electric — the external OpenAI/Anthropic façade, channels) stay in the app *above* the SDK boundary. *(This supersedes an earlier instinct to split the backend into ~12 domain packages — a folder reshuffle, not a thin core; references: nanoclaw, Eve.)*

The repo itself must be as agent-operable as the kernel is thin: a coding agent should be able to read the root instructions, run one deterministic setup/resume path, understand which files own each capability, and make local modifications without spelunking through hidden runtime state. The NanoClaw lesson is adopted as **agent-native bootstrap + skills-on-demand customization**, not as "no config": Pawrrtal keeps typed config files, validates them, and documents exactly when an agent should edit config versus code.
...
2. **Given** the kernel's dependency graph, **When** examined, **Then** it imports no concrete provider/channel/tool and no app/runtime module — only the contracts, the narrow ports, and the **Pi harness it wraps** (the one sanctioned core dependency); the build **fails** if the app→SDK arrow is reversed.
...
5. **Given** a fresh checkout and the documented operator prerequisites, **When** Claude Code/Codex follows the repo instructions, **Then** it can bootstrap, diagnose/resume a failed setup, add one requested capability, and verify the result without undocumented maintainer knowledge.
...
- **Concurrent writes to one conversation** → multiple channels (web / Telegram / Google Chat) sending to one conversation at once → the per-conversation **Rivet actor is the single writer** of session state, so concurrent inbound messages are **serialized by the actor** (no file lock, no last-writer-wins race); the cross-cutting Postgres record is written only by the API (its sole writer), so neither store can split-brain.
...
- **FR-009**: An **independently-buildable SDK boundary** (kernel + contracts + ports + `@clients/*` + the `paw` agent command group) MUST be able to **scaffold and run a standalone agent with NO Pawrrtal services** — no gateway, no profiles, no Tailscale, and no Pawrrtal persistence substrate (no Rivet actor / Postgres / Electric / Hatchet) — over a **local FileStore** (the SDK's reference store) + one provider only. The **build MUST FAIL if the SDK (kernel/contracts/ports/clients) imports any app/runtime module** — the app→SDK dependency arrow is one-way and build-enforced. The SDK remains an **internal workspace-protocol, publishable-shaped** package set in v1; **npm publishing is deferred, not chosen** (see Constraints and Out of Scope).
- **FR-053**: The repository MUST teach coding agents to bootstrap and modify Pawrrtal end-to-end: root README/AGENTS instructions, repo-local skills, and the `paw` CLI MUST provide a deterministic fresh-setup path, a resumable failure-recovery path, and a skills-on-demand customization path that copies or wires only the requested capability modules.
- **FR-054**: Configuration MUST remain explicit and durable through typed, schema-backed config files with generated examples and validation gates. Agent customization MAY edit config files, but every config change MUST be validated by the relevant setup/doctor/check command and MUST avoid plaintext secrets.
...
- **FR-021**: The sandbox substrate MUST be pluggable with a **self-hosted / own-infra default**. The default tier MUST be a **local OS-confined tier** (`local-confined` — constrained CWD + network-off via OS primitives, **no container/microVM/image**). **microVM-class tiers** (Docker + gVisor, Kata, E2B, **Gondolin** — a self-hosted Alpine micro-VM) MUST be **opt-in**, selectable per conversation; any hosted/managed substrate (e.g. Upstash Box) is **opt-in only**. The port's `pause`/`resume` is **provider-agnostic and best-effort** (live memory+FS snapshot where supported, else disk-checkpoint + cold resume), so any sandbox provider can sit behind it.
...
- **Profile**: the identity record that **replaces account login** — a Postgres row (part of the API-owned cross-cutting record, Electric-synced) holding `{ id, display name, avatar?, optional password hash + params, preferences/personalization/appearance }`. The user selects a profile; an optional password unlock yields a short-lived opaque bearer for password-locked profiles. The per-profile secret lives server-side only; the client stores only the returned token.
...
- **Agent bootstrap contract**: the documented README/AGENTS/skills/CLI path that takes a coding agent from fresh checkout to working private instance, including recovery from failed setup and on-demand capability installation.
- **Typed config file**: a durable, schema-backed operator configuration file that agents may edit when customization is configuration rather than code; secrets remain references resolved at run time.
...
- **SC-020**: A `paw new`-scaffolded **two-file** project (`instructions.md` + `agent.ts`) runs via `paw run` over a local FileStore (the SDK's reference store) + one provider with **ZERO Pawrrtal services** (no gateway, profiles, Tailscale, or persistence substrate — no Rivet actor / Postgres / Electric / Hatchet).
- **SC-021**: From a fresh checkout, a coding agent can follow the root instructions to run setup, recover from an intentionally interrupted setup, install or wire one requested capability, validate typed config, and produce a first verified message without undocumented maintainer intervention.
...
- Core substrate = **adopt the Pi package set, wrapped in Effect v4** (all MIT, `@earendil-works/*`): **`pi-agent-core`** (the agent loop + `AgentTool` interface), **`pi-ai`** (the unified multi-provider LLM/model layer — **this replaces `@effect/ai`, which is dropped, not a reference**), **`pi-tui`** + **`pi-coding-agent`** (the **default TUI** + session/history + built-in tools + extension system). Skip `pi-orchestrator` (unpublished/experimental). **Pi ships no MCP** — MCP is added as a Pi extension/tool that surfaces remote tools as `AgentTool[]`. **No external AI Gateway as a default dependency**; Pawrrtal exposes its own gateway, and non-HTTP/CLI providers stay Pawrrtal-native.
...
- Agent-operable repo posture: Pawrrtal adopts NanoClaw's "the repo teaches Claude Code how to set up and customize it" lesson as a hard requirement. The deterministic path is script/CLI-driven; the judgment path is documented for coding agents; customization is skills-on-demand and small-module/config-file edits. Pawrrtal deliberately keeps typed config files, so "easy to modify" means **validated config + small code modules**, not deleting configuration entirely.
...
- **Persistence: a four-store substrate (ADR 2026-06-27, spike-validated).** A **per-conversation Rivet actor** (running **Pi unforked**) owns the live transcript + turn/queue/per-session-cron state in its own on-disk store and streams it over its built-in WebSocket; the actor schedules its **own per-session wakes** (`c.schedule.after`), durable across cold restarts and caught up on rehydration. **Postgres** holds the cross-cutting queryable record (conversation list/metadata, profiles, projects, automations, integrations, settings, search) with the **API as sole writer**; **Electric** syncs those Postgres read-models to every device through an **identity-scoped gatekeeper** (server-forced per-owner `where` + table/column allowlist, identity from a trusted session authority, never a client header). **Hatchet** runs system-level durable work in a separate `apps/worker`, reaching a conversation only by messaging its actor. **Single-writer-per-store** is the load-bearing invariant (actor owns its session, API owns Postgres, actor↔API over RPC; no split-brain). Search depth defaults to shallow conversation **metadata** (the proven projection shape); a projected message-text **digest** or **full mirror** is a per-slice write-cost decision, no longer a substrate risk. Pinned: `rivetkit` 2.3.2 · `pi` 0.80.2 · `@electric-sql/client` 1.5.13 · `@hatchet-dev/typescript-sdk` 1.24.3 · `effect` 4.0.0-beta.74 · Postgres 17. The `SessionStore`/`EventStreamStore` ports are unchanged; only their realization is fixed (per-actor store + API-written Postgres projection). The SDK's standalone reference impl stays a simple **local store**, so a `paw new` project still runs with no actor/Postgres/Electric.
...
- Adoptions identified by prior research are treated as design inputs (not separate stories): the **message-parts data model**, **resumable streams**, the **Pi package set** (`pi-ai` · `pi-agent-core` · `pi-tui` · `pi-coding-agent`, Effect-wrapped) **+ optional gateway routing**, and **Eve-style conventions** (capability-as-file/package, skills-on-demand, channel-as-thin-adapter, durable sessions, sandboxed execution). Two Earendil repos are **reference implementations**, not direct deps: **`gondolin`** (its `VM` API is the model for the opt-in self-hosted micro-VM sandbox driver) and **`pi-chat`** (its edit-in-place streaming preview + per-conversation `ConversationSandbox` wiring are the worked example for channels + the sandbox port).
