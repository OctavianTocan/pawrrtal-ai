...
> **⚠️ SUPERSEDED — persistence/runtime substrate (2026-06-27).** Every "pure files, no database" statement in this spec (the 2026-06-23 persistence clarification, the Concept/Vision persistence bullet, the files-first concurrency note, the `$PAWRRTAL_DATA`/FileStore references) is **superseded** by the ADR `frontend/content/docs/handbook/decisions/2026-06-27-rivet-postgres-electric-hatchet-substrate.mdx`. New substrate: **per-conversation Rivet actor** (running **Pi unforked**) for live session state · **Postgres** as the API-owned queryable record · **Electric** for read-path sync to all devices · **Hatchet** for system-level durable work. The SDK-boundary, kernel-wraps-Pi, ports, profiles/Tailscale-identity, and sandbox decisions are **unchanged** — only the persistence realization moves off files. The standalone-SDK acceptance criteria (a `paw new` two-file project running over a **local store** with no Pawrrtal services) still hold; "FileStore" there means the SDK's local reference store, not the app's substrate.
...
- **Files-first concurrency** → multiple channels (web / Telegram / Google Chat) appending to one conversation at once → ~~appends to `messages.jsonl` are **`O_APPEND`-safe under a per-conversation advisory lock**, and `meta.json` mutations are **atomic temp+rename** (last-writer-wins).~~ *(⚠️ SUPERSEDED 2026-06-27: the per-conversation **Rivet actor is the single writer** of session state, so concurrent channel appends are serialized by the actor — no file lock / temp+rename needed.)*
