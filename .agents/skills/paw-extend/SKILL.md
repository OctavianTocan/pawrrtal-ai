...
│   ├── dev/                 # `paw dev up/down/status` (orchestrator package)
...
│   ├── mirror/              # `paw mirror --upstream URL COMMAND...` (orchestrator package)
...
Template to copy: `backend/app/cli/paw/commands/fanout.py` (subprocess + per-slot env), `backend/app/cli/paw/commands/dev/` (pid file + signal handling), and `backend/app/cli/paw/commands/mirror/` (SSE diff orchestration).
...
When you ship a new verb, update `.agents/skills/paw/SKILL.md`:
