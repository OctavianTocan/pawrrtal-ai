...
export function AppPill({ tone, shape, children, className }: AppPillProps): React.JSX.Element {
  const toneClasses = shape === 'tag' && tone === 'neutral' ? 'bg-foreground/[0.04]' : TONE_CLASS[tone];
  return (
    <span className={cn('inline-flex items-center justify-center', SHAPE_CLASS[shape], toneClasses, className)}>
      {children}
    </span>
  );
}
