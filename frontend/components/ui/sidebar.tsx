...
function SidebarProvider({
  ...
  const [_state, _setState] = React.useState<'expanded' | 'collapsed'>(() => (defaultOpen ? 'expanded' : 'collapsed'));
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof TopBarButton>) {
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <TopBarButton
          ...
          {...props}
        >
          <HugeiconsIcon icon={SidebarActionIcon} size={16} strokeWidth={1.7} aria-hidden="true" />
...
function SidebarMenuSkeleton({
  ...
  return (
    ...
    >
      ...
      <Skeleton className="h-4 flex-1 rounded-md" data-sidebar="menu-skeleton-text" style={{ maxWidth: width }} />
    </div>
  );
}
