...
export function EntityRow({
  ...
  const { isDraggable = false, isInMultiSelect = false, isSelected = false, showSeparator = false } = state ?? {};
  ...
  const InnerContent = (
    <div className="relative group/row select-none pl-2 mr-2">
      {(isSelected || isInMultiSelect) && <div className="absolute left-0 inset-y-0 w-[2px] bg-accent" />}
        ...
        <div className="flex flex-col gap-1.5 min-w-0 flex-1">
          {titleTrailing ? (
            <div className="flex items-center gap-[10px] w-full min-w-0">
              ...
              <div className={cn('font-sans truncate min-w-0 flex-1', titleClassName)}>{title}</div>
              <div className="shrink-0 ml-auto relative -mr-1">
                <span className={cn(menuOpen || contextMenuOpen ? 'invisible' : 'group-hover/row:invisible')}>
                  {titleTrailing}
                </span>
            ...
            <div className={cn('flex items-center gap-[10px] w-full min-w-0', icon && 'pr-6')}>
              {icon && <div className="shrink-0 flex items-center gap-[10px] [&>*]:w-3 [&>*]:h-3">{icon}</div>}
