...
async function highlightCode(code: string, language: BundledLanguage): Promise<[HighlightedCode, HighlightedCode]> {
...
function HighlightedCodeView({ className, highlighted, showLineNumbers }: HighlightedCodeViewProps): React.JSX.Element {
  return (
    <div className={className}>
      <pre className="m-0 bg-background! p-4 text-foreground! text-sm">
        <code className="font-mono text-sm">
          {highlighted?.tokens.map((line, lineIndex) => (
            <span className="block min-h-[1lh]" key={`line-${lineIndex}-${line.map((t) => t.content).join('')}`}>
              {showLineNumbers ? (
              ...
              {line.map((token) => (
                <span key={`${token.content}-${token.color ?? ''}`} style={tokenStyle(token)}>
                  {token.content}
                </span>
              ))}
            </span>
...
export const CodeBlockCopyButton = ({
  ...
  return (
    <Button className={cn('shrink-0', className)} onClick={copyToClipboard} size="icon" variant="ghost" {...props}>
      {children ?? (isCopied ? <CheckIcon aria-hidden="true" size={14} /> : <CopyIcon aria-hidden="true" size={14} />)}
    </Button>
  );
};
