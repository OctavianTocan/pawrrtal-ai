 ...
 */
export function StepMessaging({ profile, onPatch, onFinish }: StepMessagingProps): React.JSX.Element {
  ...
  const toggleChannel = (id: MessagingChannelId): void => {
    ...
    const next = connected.includes(id) ? connected.filter((entry) => entry !== id) : [...connected, id];
