...
describe('StepMessaging', () => {
  it('renders every channel as a Connect row', () => {
    const { getByText } = render(
      <StepMessaging onFinish={vi.fn()} onPatch={vi.fn()} profile={{ connectedChannels: [] }} />,
  ...
  it('disables Continue until at least one channel is connected', () => {
    ...
    rerender(<StepMessaging onFinish={vi.fn()} onPatch={vi.fn()} profile={{ connectedChannels: ['slack'] }} />);
    expect((getByRole('button', { name: 'Finish messaging setup' }) as HTMLButtonElement).disabled).toBe(false);
  });
  ...
  it('toggles a channel on Connect click and emits the new connectedChannels list', async () => {
    ...
    const { getAllByRole } = render(
      <StepMessaging onFinish={vi.fn()} onPatch={onPatch} profile={{ connectedChannels: [] }} />,
      { wrapper: createWrapper() }
    );
  ...
  it('fires onFinish when Continue is clicked while at least one channel is connected', async () => {
    ...
    const { getByRole } = render(
      <StepMessaging onFinish={onFinish} onPatch={vi.fn()} profile={{ connectedChannels: ['slack'] }} />,
      { wrapper: createWrapper() }
    );
