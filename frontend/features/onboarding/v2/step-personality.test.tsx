...
describe('StepPersonality', () => {
  it('renders every personality option as a card', () => {
    const { getByText } = render(<StepPersonality onContinue={vi.fn()} onPatch={vi.fn()} profile={{}} />);
  ...
  it('patches profile.personality when a different option is clicked', async () => {
    ...
    const { getByText } = render(<StepPersonality onContinue={vi.fn()} onPatch={onPatch} profile={{}} />);
  ...
  it('fires onContinue when the footer Continue button is clicked', async () => {
    ...
    const user = userEvent.setup();
    const { getByRole } = render(<StepPersonality onContinue={onContinue} onPatch={vi.fn()} profile={{}} />);
