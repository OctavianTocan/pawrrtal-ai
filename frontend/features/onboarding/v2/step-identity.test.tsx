...
describe('StepIdentity', () => {
  ...
  it('patches the profile when typing into the name field', async () => {
    ...
    const { getByPlaceholderText } = render(<StepIdentity onContinue={vi.fn()} onPatch={onPatch} profile={{}} />);
  ...
  it('toggles a goal chip on click and emits the new goals array', async () => {
    const onPatch = vi.fn();
    const user = userEvent.setup();
    const { getByText } = render(<StepIdentity onContinue={vi.fn()} onPatch={onPatch} profile={{ goals: [] }} />);
  ...
  it('fires onContinue when the footer button is clicked', async () => {
    ...
    const user = userEvent.setup();
    const { getByRole } = render(<StepIdentity onContinue={onContinue} onPatch={vi.fn()} profile={{}} />);
