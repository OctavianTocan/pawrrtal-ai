...
const onboardingModalReducer = (state: OnboardingModalState, action: OnboardingModalAction): OnboardingModalState => {
  switch (action.type) {
    case 'set-open':
...
export function OnboardingModal({
  ...
  const accessibleTitle =
    step === 'welcome' ? 'Welcome to Pawrrtal' : step === 'create' ? 'Create workspace' : 'Local workspace';
      ...
      >
        ...
        */}
        ...
        <div className="relative z-10 flex min-h-full items-start justify-center px-4 py-8 sm:items-center sm:px-8 sm:py-20">
          {step === 'welcome' ? (
            <OnboardingWelcomeStep onContinue={() => dispatch({ type: 'set-step', step: 'create' })} />
          ) : null}
          {step === 'create' ? (
            <OnboardingCreateWorkspaceStep onPickLocal={() => dispatch({ type: 'set-step', step: 'local' })} />
          ) : null}
          {step === 'local' ? (
