...
describe('CollapsibleGroupHeader', () => {
  it('renders the label and an item count when collapsed', () => {
    const { container } = render(
      <CollapsibleGroupHeader isCollapsed itemCount={4} label="Today" onToggle={() => undefined} />
    );
  ...
  it('hides the count when expanded', () => {
    const { container } = render(
      <CollapsibleGroupHeader isCollapsed={false} itemCount={4} label="Yesterday" onToggle={() => undefined} />
    );
  ...
  it('fires onToggle when clicked', async () => {
    ...
    const { getByRole } = render(
      <CollapsibleGroupHeader isCollapsed itemCount={3} label="Today" onToggle={onToggle} />
    );
