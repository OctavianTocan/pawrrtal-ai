...
describe('CollapsibleGroupHeader', () => {
  it('renders the label and an item count when collapsed', () => {
    const { container } = render(
      <CollapsibleGroupHeader label="Today" isCollapsed itemCount={4} onToggle={() => undefined} />
    );
  ...
  it('hides the count when expanded', () => {
    const { container } = render(
      <CollapsibleGroupHeader label="Yesterday" isCollapsed={false} itemCount={4} onToggle={() => undefined} />
    );
