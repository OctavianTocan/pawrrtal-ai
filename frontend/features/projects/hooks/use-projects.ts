 ...
 */
export function useAssignConversationToProject(): UseMutationResult<void, Error, AssignConversationVariables> {
  const fetcher = useAuthedFetch();
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: ['conversations', 'assign-project'],
    mutationFn: async ({ conversationId, projectId }: AssignConversationVariables): Promise<void> => {
