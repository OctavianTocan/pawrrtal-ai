...
export function PersonalizationSection(): React.JSX.Element {
  const profile = useSyncExternalStore(subscribeToProfile, getClientProfileSnapshot, getServerProfileSnapshot);
  ...
  return (
    ...
    >
      <SettingsCard>
        <SettingsSectionHeader description="Default tone applied to every response." title="Personality" />
        <SettingsRow description="Choose a default tone for your agent's responses." label="Personality">
          <SelectButton
      ...
      <SettingsCard>
        ...
        >
          <Switch aria-label="Enable memories" checked={enableMemories} onCheckedChange={setEnableMemories} />
        </SettingsRow>
        <SettingsRow
          description="Do not generate memories from chats that used MCP tools or web search."
          label="Skip tool-assisted chats"
        >
          <Switch aria-label="Skip tool-assisted chats" checked={skipToolChats} onCheckedChange={setSkipToolChats} />
        </SettingsRow>
