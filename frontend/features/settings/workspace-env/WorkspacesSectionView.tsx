...
export function WorkspacesSectionView({
  ...
  return (
    ...
    >
      ...
      >
        <SettingsSectionHeader description="Values are encrypted at rest on the server." noDivider title="API Keys" />
        <div aria-busy={isLoading} className="flex flex-col gap-4 py-2" data-testid="workspaces-section-fields">
          {keyMetas.map(({ key, label, description, placeholder, url }) => (
            <div className="flex flex-col gap-1.5" key={key}>
              ...
              <div className="relative flex items-center">
                ...
                />
                <button
                  aria-label={showTokens[key] ? `Hide ${label} value` : `Show ${label} value`}
                  ...
                  type="button"
                >
                  {showTokens[key] ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
                </button>
              </div>
      ...
      <div className="flex items-center gap-3">
        <Button className="gap-1.5" disabled={!isDirty || isSaving} onClick={onSave} type="button">
          <Save className="size-4" />
          {isSaving ? 'Saving...' : 'Save'}
        </Button>
        <Button className="gap-1.5" disabled={!isDirty || isSaving} onClick={onDiscard} type="button" variant="outline">
