...
function ResizableSidebarContent({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  const transformTransition = 'transition-transform duration-200 ease-out motion-reduce:transition-none';
  const widthTransition = 'transition-[width] duration-200 ease-out motion-reduce:transition-none';
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1">
       ...
       */}
      <div className={cn('relative h-full overflow-hidden', widthTransition)} style={{ width: outerWidth }}>
        {/*
...
export function AppShell({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  const isAppReady =
    e2eBypass ||
    (!onboardingReadiness.isLoading && !onboardingReadiness.isError && onboardingReadiness.hasWorkspaceReady);
  ...
  }, [e2eBypass, onboardingReadiness.isError, onboardingReadiness.hasWorkspaceReady, onboardingReadiness.isLoading]);
