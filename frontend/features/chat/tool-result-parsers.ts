...
function parseMemorySearch(value: unknown): MemoryResultInfo[] {
  ...
  for (const raw of value) {
    ...
    chips.push({
      meetingId,
      title,
      startTime: typeof memory.start_time_local === 'string' ? memory.start_time_local : undefined,
    });
  }
  return chips;
}
...
function parseChatHistory(value: unknown): MemoryResultInfo[] {
  ...
  for (const raw of conversations.slice(0, 5)) {
    ...
    const previewTitle =
      userMessage.slice(0, TITLE_PREVIEW_LIMIT) + (userMessage.length > TITLE_PREVIEW_LIMIT ? '...' : '');
    chips.push({
      meetingId: sessionId,
...
function _hasAnyChip(chips: ToolResultChips): boolean {
  return chips.webSources.length > 0 || chips.calendarEvents.length > 0 || chips.memoryResults.length > 0;
}
