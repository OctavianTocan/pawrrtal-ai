...
async function collectStream(stream: AsyncGenerator<ChatStreamEvent>): Promise<Array<ChatStreamEvent>> {
...
describe('useChat', (): void => {
  ...
  it('posts to the versioned chat endpoint and yields delta events', async (): Promise<void> => {
    ...
    const events = await collectStream(
      result.current.streamMessage('Hi', 'conversation-1', 'google-ai:google/gemini-3-flash-preview', 'high')
    );
  ...
  it('buffers SSE frames split across network chunks', async (): Promise<void> => {
    ...
    const events = await collectStream(
      result.current.streamMessage('Hi', 'conversation-1', 'google-ai:google/gemini-3-flash-preview', 'medium')
    );
  ...
  it('yields thinking, tool_use, tool_progress, and tool_result events alongside deltas', async (): Promise<void> => {
    vi.mocked(fetch).mockResolvedValue(
      createStreamResponse([
        ...
        'data: {"type":"tool_progress","tool_use_id":"t1","content":"fetching"}\n\n',
    ...
    const events = await collectStream(
      result.current.streamMessage('Hi', 'conversation-1', 'claude-code-pty:anthropic/claude-sonnet-4-6', 'extra-high')
    );
    ...
    expect(events).toEqual([
      ...
      { type: 'tool_progress', tool_use_id: 't1', content: 'fetching' },
  ...
  it('throws backend stream error events instead of thinking forever', async (): Promise<void> => {
    ...
    await expectRejectedMessage(
      collectStream(
        result.current.streamMessage('Hi', 'conversation-1', 'claude-code-pty:anthropic/claude-sonnet-4-6', 'low')
      ),
