...
export type VoiceRecordingStatus = 'idle' | 'requesting-permission' | 'recording' | 'transcribing' | 'error';
...
export function useVoiceTranscribe(): UseVoiceTranscribeResult {
  ...
  const startRecording = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      releaseStream();
      setStatus('error');
      const message = capturedError instanceof Error ? capturedError.message : 'Could not start recording.';
      setError(message);
  ...
  const stopRecording = useCallback(async (): Promise<string | null> => {
    ...
    const { transcript, errorMessage } = await requestVoiceTranscription(fetcher, finalBlob, mimeTypeRef.current);
