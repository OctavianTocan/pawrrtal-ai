...
export function ChoiceGroupRenderer(raw: LooseProps): ReactNode {
  ...
  const onSubmit = (): void => {
    ...
    const payloadValue: ChatArtifactInteractionValue = props.multi ? [...selected] : (selected[0] ?? '');
    ...
    <div className="artifact-choice-group">
      ...
      <div className="artifact-choice-options">
        {props.options.map((option) => {
          ...
          return (
            <label className="artifact-choice-option" htmlFor={inputId} key={option.value}>
              <input
  ...
  return (
    <div className="artifact-number-field">
      ...
      <input
        ...
        onChange={(e) => setValue(clampNumber(Number(e.target.value), props.min, props.max))}
        step={step}
