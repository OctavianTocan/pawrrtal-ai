  ...
  ColumnList: ({ props }) => (
    <div className={cls('artifact-col', `artifact-col-${props.kind}`)}>
      <h3 className="artifact-col-title">
        <span className={cls('artifact-tag', `artifact-tag-${props.kind}`)}>{props.kind}</span> {props.title}
      </h3>
      <ul>
  ...
  RiskGrid: ({ props }) => (
    <div className="artifact-risks">
      {props.items.map((it: { kind: 'ok' | 'warn' | 'bad'; title: string; body: string }) => (
        <div className={cls('artifact-risk', `artifact-risk-${it.kind}`)} key={`${it.kind}-${it.title}-${it.body}`}>
          <h4>{it.title}</h4>
          <p>{it.body}</p>
        </div>
      ))}
