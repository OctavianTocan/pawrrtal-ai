...
function ReasoningPanel({
  ...
  return (
    <Collapsible className="not-prose max-w-prose" onOpenChange={setIsOpen} open={isOpen}>
      ...
      >
        <div className="pt-2">
          <ChainOfThought chipsByToolId={chipsByToolId} timeline={timeline} toolCallsById={toolCallsById} />
        </div>
      </CollapsibleContent>
    </Collapsible>
  );
      ...
      <div className="flex-1">
        ...
        {onRetry ? (
          <Button
            ...
            variant="ghost"
          >
            <RefreshCwIcon className={cn('size-3.5', isRetrying ? 'animate-spin' : null)} />
...
export function AssistantMessage({
  ...
  const chipsByToolId = useMemo(() => {
    ...
    for (const call of toolCalls ?? []) {
      map.set(call.id, call.result === undefined ? EMPTY_CHIPS : extractToolChips(call.name, call.result));
    }
    return map;
  }, [toolCalls]);
  ...
  return (
    <Message from="assistant">
      <MessageContent>
        ...
        {isFailed && hasContent ? (
          <FailedReplyBanner content={content} isRetrying={isRegenerating} onRetry={onRegenerate} />
        ) : null}
