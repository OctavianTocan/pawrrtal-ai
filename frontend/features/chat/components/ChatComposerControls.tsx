...
export function AutoReviewSelector(): React.JSX.Element {
  ...
  const { menuOpen, tooltipOpen, handleMenuOpenChange, handleTooltipOpenChange } = useTooltipDropdown();
          ...
          <span className="inline-flex">
            <DropdownMenu
              ...
              renderItem={(mode, _isSelected, onSelect) => (
                <SafetyModeMenuItem isSelected={mode === safetyMode} mode={mode} onSelect={onSelect} />
              )}
              trigger={
                <Button
                  className={cn(
                  ...
                  variant="ghost"
                >
                  <ActiveIcon aria-hidden="true" className={cn('size-3.5', activeMeta.iconClass)} />
...
function SafetyModeMenuItem({ mode, isSelected, onSelect }: SafetyModeMenuItemProps): React.JSX.Element {
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2">
        <span
          aria-hidden="true"
          className={cn('inline-flex size-5 items-center justify-center rounded-[5px]', bgClass, colorClass)}
        >
