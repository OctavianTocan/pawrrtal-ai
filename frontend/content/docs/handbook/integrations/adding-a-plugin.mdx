...
> This page documents the current static plugin registry. The replacement plan
> is the [Capability Plugin Platform Implementation Plan](./capability-plugin-platform-implementation).

Plugins are additive integrations that contribute agent tools at runtime.
The Notion plugin
(`backend/app/plugins/notion/`) is the canonical example; this page
...
`backend/app/plugins/<plugin_id>/`. The package exports a single
...
from app.plugins.my_plugin.tools import factories
...
effect. Make `backend/app/plugins/__init__.py` import your
...
returns an
[`AgentTool`](../../../../../backend/app/core/run_model_tool_loop/types.py).
...
```python
# backend/app/plugins/my_plugin/tools/__init__.py
from app.plugins.my_plugin.tools.search import make_search_tool
...
[`with_audit`](../../../../../backend/app/plugins/notion/audit.py)
