...
- **LCM** (per-conversation): summary nodes, ranked lexical search,
  embeddings. Backed by `backend/app/lcm/` and exposed to the
...
- `backend/app/lcm/background.py` — the post-turn hook pattern
  to mirror
- `backend/app/agents/tool_surface.py:258-278` — where the
