...
description: Superseded ADR — the Python-native plugin registry and Notion wrapper are replaced by Plugin System V2.
status: Superseded
---
...
<Callout type="warning" title="Status: Superseded">
This ADR is superseded by [Plugin System V2](./2026-06-06-plugin-system-v2). The V2 plan removes the Python-only plugin registry, migrates Notion into the manifest-backed plugin host, and does not preserve backward compatibility with the old plugin API.
</Callout>
...
1. The agent's tool surface (`backend/app/agents/tool_surface.py:42`) is one hardcoded function that knows about every tool. Adding a Notion integration today means editing it, editing `OVERRIDABLE_KEYS`, editing the Settings frontend mirror, and editing the chat router. Four touchpoints for one capability.
2. Workspaces (`backend/app/models.py:269` — `Workspace`) are per-user-many, but workspace `.env` overrides (`backend/app/infrastructure/keys.py`) are keyed by `user_id`. A user with two workspaces shares one set of API keys across both, which contradicts the "workspace as isolated agent home" model.
...
`workspace.path` is already the workspace directory (`backend/app/workspace/filesystem.py`). The encrypted env file moves from `{workspace_base_dir}/{user_id}/.env` to `{workspace.path}/.env` — sitting next to `AGENTS.md`, `memory/`, `skills/`.
...
The plugin lives at `backend/app/plugins/notion/`. It registers 18 tools matching openclaw-notion's contract exactly (same names, same parameter shapes, same return format), so anything written against that contract works against Pawrrtal with no change.
...
`NOTION_API_KEY` is added to `OVERRIDABLE_KEYS` in `backend/app/infrastructure/keys.py` and mirrored in `frontend/features/settings/workspace-env/use-workspace-env.ts`. Reuses the existing workspace-env settings form — one labeled row, link to `https://www.notion.so/profile/integrations` for the user to mint the token.
...
## Update (2026-05-20) — collapse 18 wrappers into one `ntn` tool

The 18-tool layer that mirrored openclaw-notion's contract turned out to add cost without paying for itself: every wrapper had its own JSON-schema and argument shaping that could drift from the underlying CLI, and the agent was never reaching subcommands we hadn't pre-wired — it was just running fewer of them. The plugin now exposes a single `ntn` tool that takes an `args` array (and optional `stdin`) and pipes the resulting stdout / stderr back to the agent. The official CLI is the contract; we don't maintain a parallel one.

What stayed:

- Workspace-scoped `NOTION_API_TOKEN` injection + ephemeral `HOME` per call (`ntn_client.call_ntn`).
- The `notion_operation_logs` audit table (alembic `013`) and the `with_audit` wrapper. New rows carry `tool_name="ntn"` and `operation="cli"`; historical rows from the per-tool era keep their original `tool_name`.
- Plugin activation predicate (workspace `NOTION_API_KEY` must resolve) and Settings UI row.

What's gone:

- `backend/app/plugins/notion/tools/` (the per-category factory files and `_helpers.py`).
- The agent-facing `notion_help`, `notion_doctor`, and `notion_logs_read` surfaces. Operators inspect `notion_operation_logs` directly; the agent runs `ntn --help` if it wants to discover commands.
- `call_ntn_json` / `call_ntn_text` / `format_query_params` (unused once the wrappers were removed).

The mapping table above (`notion_search` → `ntn api v1/search …`, etc.) reads as a useful reference for what the agent can still drive through the `ntn` tool, but the LHS column no longer corresponds to live tool names.
