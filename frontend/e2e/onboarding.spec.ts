...
test.describe('onboarding wizard', () => {
  test.beforeEach(async ({ context }) => {
    const response = await context.request.post(`${process.env.E2E_API_URL ?? 'http://localhost:8000'}/auth/dev-login`);
  ...
  test('Continue button progresses to the context step', async ({ page }) => {
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.getByRole('heading', { name: /Let's give your agent some context about you/i })).toBeVisible({
      timeout: 5_000,
    });
  });
});
