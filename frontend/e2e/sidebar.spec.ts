...
test.describe('sidebar', () => {
  test('renders the seeded conversation in the sidebar', async ({ page, authenticatedPageWithWorkspaceAndChat }) => {
    ...
    const [conversationsResponse] = await Promise.all([
      page.waitForResponse(
        (resp) => resp.url().includes('/api/v1/conversations') && resp.request().method() === 'GET',
        { timeout: 15_000 }
      ),
      page.goto('/'),
    ]);
  ...
  test('renders the user profile section in the sidebar footer', async ({ page, authenticatedPageWithWorkspace }) => {
