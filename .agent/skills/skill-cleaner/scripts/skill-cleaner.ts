...
function skillRootScope(root: string): string {
  ...
  if (normalized.includes('/.agent/skills')) return 'repo';
...
function deletePriority(skill: Skill): number {
  ...
  if (skill.realPath.includes('/.agent/skills/')) return 5;
...
function discoverRoots(): string[] {
  ...
  if (exists(projects)) {
    for (const entry of fs.readdirSync(projects, { withFileTypes: true })) {
      ...
      const agentSkillRoot = path.join(projects, entry.name, '.agent/skills');
      const legacySkillRoot = path.join(projects, entry.name, '.agents/skills');
      const skillRoot = exists(agentSkillRoot) ? agentSkillRoot : legacySkillRoot;
      if (exists(skillRoot)) {
...
function collectWorkingDirectories(value: unknown): string[] {
  ...
  if (!value || typeof value !== 'object') return [];
  const record = value as Record<string, unknown>;
  const current = [record.workdir, record.cwd].filter((item): item is string => typeof item === 'string');
...
function applyUserMentions(userText: string, aliases: Map<string, string[]>, usage: Map<string, Usage>): void {
  ...
  const textCounts = countTokens(
    [...userText.matchAll(/\b(?:use|using|load|read)\s+`?\$?([A-Za-z][A-Za-z0-9_.:-]{1,80})`?/gi)].map((match) =>
      (match[1] ?? '').toLowerCase()
    )
  );
...
function skillBudget(skills: Skill[], metadataOverheadTokens = 0): Budget {
  ...
  const effectiveBudgetTokens = effectiveContextTokens ? Math.floor(effectiveContextTokens * (percent / 100)) : null;
...
function liveMetadataOverhead(live: LivePrompt | null): number {
  if (!live) return 0;
  return [...live.roots.entries()].reduce((sum, [alias, root]) => sum + tokenCost(`- \`${alias}\` = \`${root}\`\n`), 0);
}
...
function render(
  ...
  const byBase = [...groupBy(duplicatePool, (skill) => skill.baseName.toLowerCase()).entries()].filter(
    ([, list]) => list.length > 1
  );
  const byBody = [...groupBy(duplicatePool, (skill) => skill.bodyHash).entries()].filter(
...
function main(): void {
  ...
  const selectedSkills = liveSkills.length > 0 ? liveSkills : skills.filter((skill) => skill.enabled || includeAll);
